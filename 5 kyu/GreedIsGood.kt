...
fun score(dice: IntArray): Int {
    val counts = dice.toList().groupingBy { it }.eachCount()
    return counts.entries.sumBy { (num, count) ->
        when (num) {
            1 -> (count / 3) * 1000 + (count % 3) * 100
            5 -> (count / 3) * 500 + (count % 3) * 50
            else -> (count / 3) * num * 100
        }
    }
}