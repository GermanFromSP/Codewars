...
Task
Write function max_rot(n) which given a positive integer n returns the maximum number you got doing rotations similar
...
fun maxRot(n: Long): Long {
    ...
    for (i in current.indices) {
        current = current.substring(0, i) + current.substring(i + 1) + current[i]
        maxNumber = maxOf(maxNumber, current.toLong())
    }
